#!/usr/bin/env python3
# ref: https://github.com/qutebrowser/qutebrowser/blob/main/misc/userscripts/qute-lastpass
...
def pass_(domain, encoding):
    ...
    args = ['/opt/homebrew/bin/lpass', 'show', '-x', '-j', '-G', '{:s}'.format(domain)]
...
def main(arguments):
    ...
    domain_parts = urlparse(arguments.url).netloc.split(".")
    ...
    candidates = []
    seen_id = set()
    for target in ['.'.join(domain_parts[i:]) for i in range(len(domain_parts) - 1)]:
